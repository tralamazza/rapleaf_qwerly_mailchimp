...
function RapLeaf(config, db) {
  this.rapleaf = new RapLeafAPI(config.rapleaf.key); // RapLeaf API
...
RapLeaf.prototype.fetch = function() {
  var self = this;
  self.members.find({}, { email: 1 }).toArray(function(err, docs) { // just emails
    if (err) throw '[mongodb] Error fetching members. ' + err;

    var arr_emails = []
    for (var i = 0; i < docs.length; ++i)
      arr_emails.push(docs[i].email); // build an array
